...
import { ownerAdminFallbackEmail } from "@/lib/platform-config";

const cookieName = "realtyos_session";
const maxAgeSeconds = 60 * 60 * 12;
function defaultOwnerAdminEmails() {
  return [ownerAdminFallbackEmail()];
}
...
export async function createStaffSession(user: { id: string; agency_id: string; full_name: string; email: string; role: StaffRole }) {
  ...
  await query(
    `insert into staff_sessions (staff_id, agency_id, selected_agency_id, token_hash, expires_at)
     values ($1, $2, $2, $3, $4)`,
...
export async function getStaffSession(): Promise<AuthSession | null> {
  ...
  const row = await first<SessionRow>(
    `select s.staff_id,
            s.agency_id as home_agency_id,
            case when u.role = 'platform_admin' then coalesce(s.selected_agency_id, s.agency_id) else s.agency_id end as agency_id,
            u.full_name,
            u.email,
            u.role,
            u.active,
  ...
  return {
    ...
    homeAgencyId: row.home_agency_id,
    role: row.role,
    name: row.full_name,
    email: row.email,
...
export async function switchStaffSessionAgency(token: string, agencyId: string) {
  return query(
    `update staff_sessions
     set selected_agency_id = $2
     where token_hash = $1
       and revoked_at is null
       and exists (select 1 from agencies where id = $2 and coalesce(status, 'active') = 'active')
     returning id`,
    [tokenHash(token), agencyId],
  );
}
...
export async function requireStaffSession(allowedRoles?: StaffRole[]) {
  ...
  if (allowedRoles && session.role !== "platform_admin" && !allowedRoles.includes(session.role)) return { response: forbidden(), session: null };
...
export function ownerAdminEmail() {
  return ownerAdminEmails()[0] ?? ownerAdminFallbackEmail();
}

export async function requireOwnerAdmin() {
  const auth = await requireStaffSession(["admin", "platform_admin"]);
  if (auth.response) return auth;

  // Managing-agent admins and platform admins are both allowed to use setup/admin tooling.
  return auth;
}

...
const creators = {
  ...
  housingAssociations: createHousingAssociation,
...
export async function POST(request: Request, context: { params: Promise<{ collection: string }> }) {
  ...
  const auth = await requireStaffSession(writeRoles[collection].POST);
  if (auth.response) {
  ...
  try {
    ...
    if (collection === "supportNotes") body.createdByUserId = auth.session?.staffId;
...
export async function PATCH(request: Request, context: { params: Promise<{ collection: string }> }) {
  ...
  const auth = await requireStaffSession(writeRoles[collection]?.PATCH);
...
export async function DELETE(request: Request, context: { params: Promise<{ collection: string }> }) {
  ...
  const auth = await requireStaffSession(writeRoles[collection]?.DELETE);

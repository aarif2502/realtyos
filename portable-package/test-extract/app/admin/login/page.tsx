...
export default function AdminLoginPage() {
  ...
  async function signIn(event: FormEvent<HTMLFormElement>) {
    ...
    setRole(payload.staff?.role === "finance" ? "accountant" : payload.staff?.role === "readonly" ? "agent" : payload.staff?.role === "admin" ? "admin" : "agent");
    router.push("/realtyos");
  }
    ...
    <main className="app-shell flex min-h-screen items-center py-10">
      <section className="glass-panel mx-auto w-full max-w-lg rounded-[34px] p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.16em] theme-muted">UK Support Housing Secure Access</p>
            <p className="text-sm font-semibold text-slate-900">RealtyOS Workspace Login</p>
          </div>
        </div>
        ...
        <form onSubmit={signIn} className="mt-6 space-y-3">
          ...
          <button className="button-primary w-full" disabled={loading}>
            {loading ? "Signing in..." : "Sign in to RealtyOS"}
          </button>
        </form>

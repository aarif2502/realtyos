...
export default function DocumentsPage() {
  ...
  async function analyzeDocument(path: string) {
    setAnalysis(null);
    const response = await fetch(`/api/documents/analyze?path=${encodeURIComponent(path)}`, { cache: "no-store" });
            ...
            return (
              <article key={document.id} className="rounded-xl border border-slate-200 bg-white p-3 text-sm">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button type="button" className="button-secondary px-4 py-2" onClick={() => void analyzeDocument(openPath)}>Analyze</button>

...
export async function GET(request: Request) {
  ...
  try {
    ...
    const contentType = document?.mime_type || contentTypeFor(target);
    return new NextResponse(body, {
      headers: {
        "Content-Type": contentType,
        "Content-Disposition": `inline; filename="${(document?.original_file_name || filePath.split("/").pop() || "document").replace(/"/g, "")}"`,
      },
    });
  } catch (error) {
